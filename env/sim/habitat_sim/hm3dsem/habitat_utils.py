...
def make_cfg(
    ...
    """
    Create a configuration for the simulator based on the input parameter
    dictionary. This function is for Habitat3DSemantic dataset.
    ...
    backend_cfg.scene_id = os.path.join(
        raw_data_dir, scene_name + ".basis.glb")
...
def load_poses_from_file(pose_file_path: str) -> List[np.ndarray]:
    """
    Load camera poses from a file. The file store a sequence of 4x4
    transformation matrices of the camera poses. Each row in the file contains
    the flatten 4x4 matrix of one camera pose.
...
def save_obs(
        ...
        save_count: int) -> None:
    """
    Save the observation dictionary (rgb, depth, semantic, etc.) as images or
    numpy arrays in separate folders.
    ...
    # save rgb
    save_name = (sim_setting["scene"].split(
        "/")[-1].split(".")[0] + f"_{save_count:06}.png")
    ...
    if sim_setting["semantic_sensor"]:
        save_name = (sim_setting["scene"].split(
            "/")[-1].split(".")[0] + f"_{save_count:06}.npy")
    ...
    # save pose
    save_name = (sim_setting["scene"].split(
        "/")[-1].split(".")[0] + f"_{save_count:06}.txt")
