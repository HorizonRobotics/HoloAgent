    ...
    for i in range(len(results)):
        if (i % 2) == 0:
            ...
            times.append(
                ...
                1e09 +
                results[i].planning_time.sec)
        else:
            # Append smoothed times array
            for result in results[i]:
                times.append(result.smoothing_duration.nanosec /
                             1e09 + result.smoothing_duration.sec)
    return times
...
def arcCenter(pt_prev, pt, pt_next):
    ...
    center = np.array([(det1 * n2[0] - det2 * n1[0]) / det,
