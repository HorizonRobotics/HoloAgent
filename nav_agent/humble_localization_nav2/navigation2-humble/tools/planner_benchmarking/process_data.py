...
def getTimes(results):
    ...
    for result in results:
        for time in result:
            times.append(
                time.planning_time.nanosec /
                1e09 +
                time.planning_time.sec)
    return times
...
def main():
    ...
    for i in range(0, len(planners)):
        planner_table.append([planners[i], np.average(path_lengths[i]), np.average(
            times[i]), np.average(average_path_costs[i]), np.average(max_path_costs[i])])
